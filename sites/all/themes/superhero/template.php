...
function superhero_preprocess_html(&$vars) {
  ...
  if ($theme->settings['responsive']) {
    drupal_add_css(drupal_get_path('theme', 'superhero') . '/vendor/bootstrap/css/bootstrap-responsive.min.css');
...
function superhero_preprocess_page(&$vars) {
  ...
  $file = $scss->outputFile($theme_path.'/css/' . $theme->theme . '-theme-preset' . ($default_preset + 1) . '.css');
...
function template_preprocess_section(&$vars) {
  ...
  $vars['attributes_array']['class'] = $vars['classes_array'];
	$custom_style = "";
	if(trim($margin)){
		$custom_style .= "margin:{$margin} !important;";
	}
	if(trim($padding)){
		$custom_style .= "padding:{$padding} !important;";
	}
	if(trim($background)){
		$custom_style .= "background-color:{$background} !important;";
	}
	if($custom_style){
		$vars['custom_style'] = " style=\"{$custom_style}\"";
	}else{
		$vars['custom_style'] = "";
	}
}
